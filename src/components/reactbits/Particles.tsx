...
const vertex = /* glsl */ `
  ...
  uniform float uMoveParticlesOnHover;
  ...
  void main() {
    ...
    if (uMoveParticlesOnHover > 0.5) {
      vec2 toMouse = uMouse - pos.xy;
      float distanceToMouse = length(toMouse);
      float force = max(0.0, 1.0 - distanceToMouse * 2.0);
      
      // Push particles away from mouse
      if (force > 0.0) {
        vec2 push = normalize(toMouse) * force * uHoverFactor;
        pos.xy += push;
      }
    }
...
const Particles: React.FC<ParticlesProps> = ({
  ...
  useEffect(() => {
    ...
    if (moveParticlesOnHover) {
      console.log('Mouse interaction ENABLED'); // Debug
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uMoveParticlesOnHover: { value: moveParticlesOnHover ? 1 : 0 }
      },
      transparent: true,
      depthTest: false
    });
